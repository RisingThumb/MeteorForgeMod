package com.risingthumb.tutorial.world.gen;
...
public class TutorialWorldGen implements IWorldGenerator {
	...
	public TutorialWorldGen() {
		tutorialOverworldGenerator = new WorldGenMinable(ModBlocks.METEOR_ORE.getDefaultState(), 15, BlockMatcher.forBlock(Blocks.STONE));
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		...
		case OVERWORLD:
			genStandard(tutorialOverworldGenerator, world, random, chunkX, chunkZ, 10, 0, 200);
			break;
	...
	private void genStandard(WorldGenerator generator, World world, Random random, int chunkX, int chunkZ, int spawnTries, int minHeight, int maxHeight) {
		...
		for (int i = 0; i < spawnTries; i++) {
			generator.generate(world, random, chunkPosAsBlockPos.add(random.nextInt(16),
